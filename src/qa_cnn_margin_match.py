...
tf.flags.DEFINE_string("answer_input", "data/qa/answer_token", "")

tf.flags.DEFINE_string("train_quesiton_id_fn", "data/qa/train_question_id", "")
tf.flags.DEFINE_string("train_question_token_fn", "data/qa/train_question_token", "")
tf.flags.DEFINE_string("validate_question_token_fn",
        "data/qa/validate_question_token", "")

tf.flags.DEFINE_string("train_json_fn", "data/qa/train.json.gz", "")
tf.flags.DEFINE_string("validate_json_fn", "data/qa/validate.json.gz", "")
...
class Model:
    ...
    def __init__(self, w2v_embedding, max_sequence_length, filter_size, num_filter,
        ...
        with tf.Graph().as_default() as graph:
            ...
            self.bias = tf.get_variable(name="bias", shape=[num_filter],
                    dtype=tf.float32,
    ...
    def inference(self, question, answer, false_answer, reuse):
        ...
        with tf.variable_scope("conv", reuse=reuse) as scope:
            question_conv = tf.nn.conv2d(question_input, self.filter, strides=[1,
               1, 1, 1], padding="VALID")
            question_conv = tf.nn.relu(question_conv + self.bias)
            ...
            answer_conv = tf.nn.conv2d(answer_input, self.filter, strides=[1,
               1, 1, 1], padding="VALID")
            answer_conv = tf.nn.relu(answer_conv + self.bias)
            ...
            if false_answer != None:
                ...
                false_answer_conv = tf.nn.relu(false_answer_conv + self.bias)
    ...
    def create_train_inputs(self, ids, questions, answers, train_json):
        ...
        for i in ids:
            ...
            if len(true_answer_ids) == 0 or len(false_answer_ids) == 0:
                qid = np.random.choice(range(7895))
                while len(train_json[str(qid)]['answers']) == 0 or len(train_json[str(qid)]['negatives']) == 0:
                    qid = np.random.choice(range(7895))

                true_answer_ids = train_json[str(qid)]['answers']
                false_answer_ids = train_json[str(qid)]['negatives']
                true_id = np.random.choice(true_answer_ids)
                false_id = np.random.choice(false_answer_ids)

                question_tokens.append(questions[int(qid)])
    ...
    def train(self, batch_size, train_question_id_fn, train_question_token_fn,
        ...
        validate_questions, validate_answers, validate_labels = insurance_loader.load_tokens_for_validate(
                            validate_question_token_fn, answer_token_fn, validate_json_fn)
        ...
        #train_question_all, train_answer_all, train_label_all = self.create_train_eval_inputs(
        #        range(2000), validate_question_tokens, validate_answer_tokens,
        #        validate_json)
        ...
        with sv.managed_session(master='') as sess:
            for step in range(self.max_train_step):
                ...
                if  (step + 1) % 1000 == 0:
                    print("all validate")
                    self.validate(sess, valid_sim, validate_questions,
...
def main(_):
    ...
    model.train(FLAGS.batch_size, FLAGS.train_quesiton_id_fn,
